...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (sAccountMgr->IsPlayerAccount(GetSecurity()) && sObjectMgr->IsReservedName(name))
    {
    ...
    if (heroic_free_slots == 0 && sAccountMgr->IsPlayerAccount(GetSecurity()) && class_ == CLASS_DEATH_KNIGHT)
    {
    ...
    if (sAccountMgr->IsPlayerAccount(GetSecurity()) && class_ == CLASS_DEATH_KNIGHT && req_level_for_heroic > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
    ...
    */
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 2:
        {
            ...
            bool allowTwoSideAccounts = !sWorld->IsPvPRealm() || sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) || !sAccountMgr->IsPlayerAccount(GetSecurity());
        ...
        case 3:
        {
            ...
            if (result)
            {
                ...
                if (sAccountMgr->IsPlayerAccount(GetSecurity()) && createInfo->Class == CLASS_DEATH_KNIGHT)
                {
            ...
            if (sAccountMgr->IsPlayerAccount(GetSecurity()) && createInfo->Class == CLASS_DEATH_KNIGHT && !hasHeroicReqLevel)
            {
                WorldPacket data(SMSG_CHAR_CREATE, 1);
                data << uint8(CHAR_CREATE_LEVEL_REQUIREMENT);
                SendPacket(&data);
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    ...
    // check name limitations
    if (sAccountMgr->IsPlayerAccount(GetSecurity()) && sObjectMgr->IsReservedName(newName))
    {
        WorldPacket data(SMSG_CHAR_RENAME, 1);
        data << uint8(CHAR_NAME_RESERVED);
        SendPacket(&data);
