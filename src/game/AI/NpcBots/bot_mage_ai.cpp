    ...
    struct mage_botAI : public bot_minion_ai
    {
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            if (IsSpellReady(LIVINGBOMB_1, diff) && HasRole(BOT_ROLE_DPS) && dist < 35 && opponent->GetHealth() > me->GetHealth()/2 &&
                Rand() < 45 && !HasAuraName(opponent, LIVINGBOMB_1, me->GetGUIDObject()) &&
        ...
        void CheckPoly(uint32 diff)
        {
            ...
            {
                Polymorph = FindAffectedTarget(GetSpell(POLYMORPH_1), me->GetGUIDObject());
                polyCheckTimer = 2000;
            }
        }
        ...
        void FocusMagic(uint32 diff)
        {
            ...
            if (Unit* target = FindAffectedTarget(FOCUSMAGIC, me->GetGUIDObject(), 70, 3))
            {
                fmCheckTimer = 15000;
                return;
            }
