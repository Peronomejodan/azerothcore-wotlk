    ...
    struct bot_druid_ai : public bot_minion_ai
    {
        ...
        void doBalanceActions(uint32 diff)
        {
            ...
            if (IsSpellReady(MOONFIRE_1, diff) && HasRole(BOT_ROLE_DPS) && Rand() < 20 &&
                !HasAuraName(opponent, MOONFIRE_1, me->GetGUIDObject()))
            {
        ...
        bool HealTarget(Unit* target, uint8 hp, uint32 diff)
        {
            ...
            if ( ( (hp < 80 || GetLostHP(target) > 3500 || tanking) &&
                regrowth && !HasAuraName(target, REGROWTH_1, me->GetGUIDObject()) )
                ||
                (HasAuraName(target, REGROWTH_1, me->GetGUIDObject()) && HasAuraName(target, REJUVENATION_1, me->GetGUIDObject()) &&
            ...
            if (GetSpell(REJUVENATION_1) && GC_Timer <= diff && hp > 25 &&
                ...
                !HasAuraName(target, REJUVENATION_1, me->GetGUIDObject()))
            {
        ...
        void CheckRoots(uint32 diff)
        {
            ...
            if (FindAffectedTarget(ENTANGLING_ROOTS, me->GetGUIDObject(), 60)) return;
        ...
        void CheckBattleRez(uint32 diff)
        {
            ...
            if (!gr)
            {
                ...
                //if (master->IsResurrectRequested()) return; //ressurected not working yet
            ...
            for (GroupReference* itr = gr->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                //if (tPlayer->IsResurrectRequested()) continue; //ressurected not working yet
