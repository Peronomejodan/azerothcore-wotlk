...
public:
    ...
    struct warlock_botAI : public bot_minion_ai
    {
        ...
        uint8 Afflicted(Unit* target)
        {
            ...
            bool aff = HasAuraName(target, UNSTABLE_AFFLICTION_1, me->GetGUIDObject());
            bool imm = HasAuraName(target, IMMOLATE_1, me->GetGUIDObject());
            if (imm) return 1;
            if (aff) return 2;
            return 0;
        }
        ...
        void CheckFear()
        {
            ...
            if (Unit* u = FindAffectedTarget(FEAR, me->GetGUIDObject()))
