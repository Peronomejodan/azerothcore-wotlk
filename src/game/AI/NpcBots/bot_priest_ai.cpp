    ...
    struct priest_botAI : public bot_minion_ai
    {
                ...
                //general rule
            {
                ...
                if (HasRole(BOT_ROLE_DPS) && dist < 30)
                {
                    ...
                    if (IsSpellReady(SW_PAIN_1, diff) && Rand() < 25 &&
                        ...
                        !HasAuraName(opponent, SW_PAIN_1, me->GetGUIDObject()) &&
                        doCast(opponent, GetSpell(SW_PAIN_1)))
                        return;
                    if (IsSpellReady(VAMPIRIC_TOUCH_1, diff) && Rand() < 50 &&
                        opponent->GetHealth() > me->GetMaxHealth()/4 &&
                        !HasAuraName(opponent, VAMPIRIC_TOUCH_1, me->GetGUIDObject()) &&
                    ...
                    if (IsSpellReady(DEVOURING_PLAGUE_1, diff) && !Devcheck && Rand() < 30 &&
                        ...
                        !HasAuraName(opponent, DEVOURING_PLAGUE_1, me->GetGUIDObject()) &&
                        doCast(opponent, GetSpell(DEVOURING_PLAGUE_1)))
                        return;
                    if (IsSpellReady(MIND_BLAST_1, diff) && Rand() < 35 &&
                        (!GetSpell(VAMPIRIC_TOUCH_1) || HasAuraName(opponent, VAMPIRIC_TOUCH_1, me->GetGUIDObject())) &&
                    ...
                    if (IsSpellReady(MIND_FLAY_1, diff, false) && Rand() < 20 &&
                        ...
                        (HasAuraName(opponent, SW_PAIN_1, me->GetGUIDObject()) && HasAuraName(opponent, DEVOURING_PLAGUE_1, me->GetGUIDObject()))) &&
        ...
        bool HealTarget(Unit* target, uint8 hp, uint32 diff)
        {
            ...
            if (IsSpellReady(RENEW_1, diff) &&
                ...
                !HasAuraName(target, RENEW_1, me->GetGUIDObject()) &&
        ...
        void CheckShackles(uint32 diff)
        {
            ...
            if (FindAffectedTarget(SHACKLE_UNDEAD, me->GetGUIDObject()))
            {
                Shackle_Timer = 1500;
                return;
            }
        ...
        void DoDevCheck(uint32 diff)
        {
            ...
            {
                Devcheck = FindAffectedTarget(GetSpell(DEVOURING_PLAGUE_1), me->GetGUIDObject());
                DevcheckTimer = 5000;
            }
        }
