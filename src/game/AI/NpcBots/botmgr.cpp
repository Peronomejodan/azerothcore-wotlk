...
typedef std::list<uint64> GuidListI;
//typedef std::list<ObjectGuid> GuidListI;
...
void BotMgr::Update(uint32 diff)
{
    ...
    {
        GuidListI::iterator itr = _removeList.begin();
...
void BotMgr::RemoveBot(ObjectGuid guid, uint8 removetype)
{
    ...
    for (GuidListI::iterator it = _removeList.begin(); it != _removeList.end(); ++it)
    {
        if (*it == guid)
        {
    ...
    if (removetype == BOT_REMOVE_DISMISS)
    {
        ...
        stmt->setUInt32(0, _owner->GetGUIDObject().GetCounter());
...
BotAddResult BotMgr::AddBot(Creature* bot, bool takeMoney)
{
    ...
    bot->SetGuidValue(UNIT_FIELD_CREATEDBY, _owner->GetGUIDObject());
