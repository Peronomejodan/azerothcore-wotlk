...
enum Spells
{
    ...
    //SPELL_PREPARATION       = 14185, declaration error
...
void SimpleCharmedPlayerAI::OnCharmed(bool apply)
{
    if (apply)
    {
        ...
        me->StopMoving();
        me->GetMotionMaster()->Clear();
        me->GetMotionMaster()->MovePoint(0, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), false); // force re-sync of current position for all clients
    }
    else
    {
        me->CastStop();
