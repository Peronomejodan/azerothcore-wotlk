...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "faction",    SEC_PLAYER,                    false, &HandleNpcSetFactionCommand,             ""},
            { "owner",      SEC_PLAYER,                      false, &HandleNpcSetOwnerCommand,               ""},
        };
        ...
        static std::vector<ChatCommand> npcbotCommandTable =
        {
            ...
            { "attack",	    SEC_PLAYER,		       false, &HandleNpcBotAttackCommand,	       ""},
            { "withdraw",   SEC_PLAYER,		       false, &HandleNpcBotWithdrawCommand,	       ""},
        };
    ...
    static bool HandleNpcBotDeleteCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (Player* botowner = bot->GetBotOwner()->ToPlayer())
            botowner->GetBotMgr()->RemoveBot(bot->GetGUIDObject(), BOT_REMOVE_DISMISS);
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), map, chr->GetPhaseMaskForSpawn(), id, 0, x, y, z, o))
        {
            delete creature;
            return false;
        }
        ...
        uint32 factionId = 0;        
        if(chr->GetTeamId() == ALLIANCE)
            factionId = 1802; //alliance
        else if(chr->GetTeamId() == HORDE)
            factionId = 1801; //horde
        else
    ...
    static bool HandleNpcBotRemoveCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (cre && cre->GetIAmABot() && !cre->IsFreeBot())
        {
            ...
            master->GetBotMgr()->RemoveBot(cre->GetGUIDObject(), BOT_REMOVE_DISMISS);
            if (master->GetBotMgr()->GetBot(cre->GetGUIDObject()) == NULL)
            {
    ...
    static bool HandleNpcBotResetCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        Player* master = NULL;
        bool all = false;
        //uint16 guidx = owner->GetTarget();
        //ObjectGuid guid =guidx->GetGUIDObject();
        ObjectGuid guid = owner->GetGUIDTarget();
        if (!guid)
        {
